...
/* add by cym 20150817 */
unsigned int sdfuse_cmd_flags = 0;
/* end add */
...
static int update_from_sd (char *part, char *file)
{
...

	{
		...
		f_status.down_bytes = download_bytes;
		
		ret = f_interface.rx_handler(command, sizeof(command));
	}
...
int do_sdfuse (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	f_interface.transfer_buffer     = (unsigned char *) CFG_FASTBOOT_TRANSFER_BUFFER;
	f_interface.transfer_buffer_size        = CFG_FASTBOOT_TRANSFER_BUFFER_SIZE;
	...
	else if ((argc == 2) && !strcmp(argv[1], "flashall"))
	{
		...
		update_from_sd("2ndboot", "2ndboot.bin");

		update_from_sd("bootloader", "u-boot.bin");

		update_from_sd("boot", "boot.img");

		update_from_sd("system", "system.img");

		update_from_sd("uaerdata", "userdata.img");

		update_from_sd("recovery", "recovery.img");
